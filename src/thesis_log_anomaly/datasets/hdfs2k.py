...
def load_hdfs2k_records(raw_dir: str | Path | None = None) -> list[LogRecord]:
    root = Path(raw_dir) if raw_dir is not None else dataset_root()
    csv_path = root / "HDFS_2k.log_structured.csv"
    if csv_path.exists():
        return _load_hdfs2k_from_csv(csv_path)
    return _load_hdfs2k_from_log(root / "HDFS_2k.log")
...
def _load_hdfs2k_from_csv(csv_path: Path) -> list[LogRecord]:
...
def _load_hdfs2k_from_log(log_path: Path) -> list[LogRecord]:
    records: list[LogRecord] = []
